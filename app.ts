const OUTPUT_SIZES = [16, 24, 32, 40, 48, 64, 96] as const;

const iconFileInput = document.getElementById("iconFile") as HTMLInputElement;
...
async function renderPngFromSource(
...
function buildIcoFile(
  items: Array<{ size: number; pngData: Uint8Array }>
): Uint8Array {
  const headerSize = 6;
  const entrySize = 16;
...
function downloadIco(bytes: Uint8Array, fileName: string): void {
...
function renderPreview(pngBuffers: Uint8Array[]): void {
